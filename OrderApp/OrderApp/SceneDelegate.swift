...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    {
        switch MenuController.shared.order.menuItmes.count
        {
        case 0 :
            orderTabBarItem.badgeValue =  nil
        case let count :
        orderTabBarItem.badgeValue = String(count)
            orderTabBarItem.badgeColor = UIColor.blue
    }
    }
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        NotificationCenter.default.addObserver(self, selector: #selector(updateOrderBagde), name: MenuController.orderUpdatedNotification, object: nil)
